                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach ($countries as $country )
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="flex items-center">{{$country->country_code}}</div>
                                                </td>
                                                <td class="flex px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <a 
                                                        href="{{ route('admin.add_state', $country->id) }}"
                                                    ...
                                                    <form method="POST" action="{{ route('admin.countries.destroy',$country->id) }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <a class="text-red-600 hover:text-red-900 px-2" href="{{ route('admin.countries.destroy',$country->id) }}"
